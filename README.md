...
- **Mac-style modifier remap via keyd** — at the evdev layer (below XKB/niri/IME), keyd remaps the Windows/Super key → Alt, Right Alt → Control, and Left Alt → Meta/Super so niri's `Mod` is triggered by Left Alt. CapsLock is `overload(control, esc)` (tap=Esc, hold=Ctrl). The Microsoft Copilot key (`f23`) → Alt via a `leftmeta+leftshift+f23 = layer(alt)` chord binding so the firmware's pre-chord meta+shift physical keydowns don't bleed through. Config source-of-truth is `docs/keyd/default.conf`; applied by `docs/keyd/apply.sh` (see `INSTALL.md` §7).
...
- **faillock policy is hand-managed, not HM** — `/etc/security/faillock.conf` has exactly two active directives vs the pristine `pam` package file: `deny = 15` (lock after 15 consecutive failed auths) and `unlock_time = 300` (5-min auto-unlock). Deliberately outside the flake: this is system-scope PAM policy and standalone HM only owns `$HOME`; a `home.activation` + `sudo tee` hack would demand passwordless sudo on every switch and leave untracked drift (HM can't manage `/etc` lifecycle). The PAM stack already loads `pam_faillock.so` (`/etc/pam.d/system-auth`), so edits take effect immediately; `even_deny_root` stays off so root can't be locked out. Inspect/reset with `faillock --user <name>` / `sudo faillock --user <name> --reset`. If NixOS is ever adopted, move to `environment.etc."security/faillock.conf"`.
